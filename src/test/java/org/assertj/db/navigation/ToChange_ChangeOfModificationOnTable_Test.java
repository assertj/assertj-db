...
public class ToChange_ChangeOfModificationOnTable_Test extends AbstractTest {
  ...
  public void test_change_of_modification_on_table_with_displays() throws Exception {
    ...
    assertThat(map).hasSize(0);
    assertThat(map.get(null)).isNull();
    ChangeDisplay changeDisplay0 = changesDisplay.changeOfModificationOnTable("actor");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(1);
    assertThat(map.get(ChangeType.MODIFICATION).get("actor")).isEqualTo(1);
    ChangeDisplay changeDisplay1 = changesDisplay.changeOfModificationOnTable("interpretation");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(2);
    assertThat(map.get(ChangeType.MODIFICATION).get("interpretation")).isEqualTo(1);
    ChangeDisplay changeDisplay2 = changesDisplay.changeOfModificationOnTable("movie");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(3);
    assertThat(map.get(ChangeType.MODIFICATION).get("movie")).isEqualTo(1);
    try {
      changesDisplay.changeOfModificationOnTable("actor");
      fail("An exception must be raised");
    } catch (AssertJDBException e) {
      Assertions.assertThat(e.getMessage()).isEqualTo("Index 1 out of the limits [0, 1[");
    }
    ...
    assertThat(map).hasSize(0);
    assertThat(map.get(null)).isNull();
    ChangeDisplay changeDisplayBis0 = changesDisplayBis.changeOfModificationOnTable("actor");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(1);
    assertThat(map.get(ChangeType.MODIFICATION).get("actor")).isEqualTo(1);
    ChangeDisplay changeDisplayBis1 = changeDisplayBis0.changeOfModificationOnTable("interpretation");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(2);
    assertThat(map.get(ChangeType.MODIFICATION).get("interpretation")).isEqualTo(1);
    ChangeDisplay changeDisplayBis2 = changeDisplayBis1.changeOfModificationOnTable("movie");
    assertThat(map).hasSize(1);
    assertThat(map.get(null)).isNull();
    assertThat(map.get(ChangeType.MODIFICATION)).hasSize(3);
    assertThat(map.get(ChangeType.MODIFICATION).get("movie")).isEqualTo(1);
    try {
      changeDisplayBis2.changeOfModificationOnTable("interpretation");
      fail("An exception must be raised");
    } catch (AssertJDBException e) {
      Assertions.assertThat(e.getMessage()).isEqualTo("Index 1 out of the limits [0, 1[");
    }
