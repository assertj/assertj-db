...
public class AssertOnValueEquality_IsEqualTo_Character_Test extends AbstractTest {
  ...
  public void test_is_equal_to() {
    ...
    ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.isEqualTo('T');
    Assertions.assertThat(changeColumnValueAssert).isSameAs(changeColumnValueAssert2);

    TableColumnValueAssert tableColumnValueAssert = assertThat(table).column("var16").value();
    TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.isEqualTo('T');
  ...
  public void should_fail_because_value_is_not_equal_to() {
    ...
    try {
      assertThat(table).column("var16").value(1).isEqualTo('t');
      fail("An exception must be raised");
    } catch (AssertionError e) {
      Assertions.assertThat(e.getMessage()).isEqualTo(String.format("[Value at index 1 of Column at index 15 (column name : VAR16) of TEST table] %n"
        + "Expecting:%n"
        + "  <\"e\">%n"
        + "to be equal to: %n"
        + "  <'t'>"));
