 ...
 * {@link org.assertj.db.api.assertions.AssertOnValueNonEquality#isNotEqualTo(Boolean)} method.
 *
 * @author Régis Pouiller
 *
 */
public class AssertOnValueNonEquality_IsNotEqualTo_Boolean_Test extends AbstractTest {
    ...
    public void test_is_not_equal_to() {
        ...
        ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.isNotEqualTo(true);
        Assertions.assertThat(changeColumnValueAssert).isSameAs(changeColumnValueAssert2);

        TableColumnValueAssert tableColumnValueAssert = assertThat(table).column("var2").value(2);
        TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.isNotEqualTo(true);
    ...
    public void should_fail_because_value_is_equal_to() {
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                "[Value at end point of Column at index 1 (column name : VAR2) of Change at index 0 (with primary key : [1]) of Changes on test table of 'sa/jdbc:h2:mem:test' source] %n"
                + "Expecting:%n"
                + "  <true>%n"
        ...
        try {
            assertThat(table).column("var2").value().isNotEqualTo(true);
            fail("An exception must be raised");
        } catch (AssertionError e) {
            Assertions.assertThat(e.getMessage()).isEqualTo(
                String.format("[Value at index 0 of Column at index 1 (column name : VAR2) of test table] %n"
                              + "Expecting:%n"
                              + "  <true>%n"
                              + "not to be equal to: %n"
                              + "  <true>"));
