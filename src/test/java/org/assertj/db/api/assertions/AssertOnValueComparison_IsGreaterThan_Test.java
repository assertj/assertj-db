...
public class AssertOnValueComparison_IsGreaterThan_Test extends AbstractTest {
    ...
    public void test_is_less_than() {
        ...
        ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.isGreaterThan(1);
        ...
        TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.isGreaterThan(1);
    ...
    public void should_fail_because_value_is_greater_than_or_equal_to() {
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                "[Value at end point of Column at index 2 (column name : VAR3) of Change at index 0 (with primary key : [1]) of Changes on test table of 'sa/jdbc:h2:mem:test' source] %n"
                + "Expecting:%n"
                + "  <2>%n"
        ...
        try {
            assertThat(table).column("var3").value().isGreaterThan(2);
            fail("An exception must be raised");
        } catch (AssertionError e) {
            Assertions.assertThat(e.getMessage()).isEqualTo(
                String.format("[Value at index 0 of Column at index 2 (column name : VAR3) of test table] %n"
                              + "Expecting:%n"
                              + "  <2>%n"
                              + "to be greater than %n"
                              + "  <2>"));
