 ...
 * {@link  AssertOnValueCondition#has(Condition)} method.
 *
 * @author Julien Roy
 *
 */
public class AssertOnValueCondition_Has_Test extends AbstractTest {
  ...
  /**
   * This method tests the {@code has} assertion method.
   */
  @Test
  @NeedReload
  public void test_has() {
    ...
    ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.has(zero);
    ...
    TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.has(zero);
  ...
  public void should_fail_because_value_not_match_with_condition() {
    ...
    try {
      assertThat(changes).change().column("var3").valueAtEndPoint().has(zero);
