...
public class AssertOnValueEquality_IsEqualTo_Bytes_Test extends AbstractTest {
    ...
    public void test_is_equal_to() {
        ...
        ChangeColumnValueAssert changeColumnValueAssert = assertThat(changes).change().column("var11")
                                                                             .valueAtEndPoint();
        ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.isEqualTo(bytesH2);
        ...
        TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.isEqualTo(bytesH2);
    ...
    public void should_fail_because_value_is_not_equal_to() {
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                "[Value at end point of Column at index 10 (column name : VAR11) of Change at index 0 (with primary key : [1]) of Changes on test table of 'sa/jdbc:h2:mem:test' source] %n"
                + "Expecting to be equal to the expected value but was not equal"));
        }
        try {
            assertThat(table).column("var11").value().isEqualTo(new byte[0]);
            fail("An exception must be raised");
        } catch (AssertionError e) {
            Assertions.assertThat(e.getMessage()).isEqualTo(
                String.format("[Value at index 0 of Column at index 10 (column name : VAR11) of test table] %n"
                              + "Expecting to be equal to the expected value but was not equal"));
        }
    }
}
