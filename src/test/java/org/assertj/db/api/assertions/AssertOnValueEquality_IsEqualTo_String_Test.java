...
public class AssertOnValueEquality_IsEqualTo_String_Test extends AbstractTest {
    ...
    public void test_is_equal_to() {
        ...
        ChangeColumnValueAssert changeColumnValueAssert = assertThat(changes).change().column("var12")
                                                                             .valueAtEndPoint();
        ChangeColumnValueAssert changeColumnValueAssert2 = changeColumnValueAssert.isEqualTo("text");
        ...
        TableColumnValueAssert tableColumnValueAssert2 = tableColumnValueAssert.isEqualTo("text");
    ...
    public void should_fail_because_value_is_not_equal_to() {
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                "[Value at end point of Column at index 11 (column name : VAR12) of Change at index 0 (with primary key : [1]) of Changes on test table of 'sa/jdbc:h2:mem:test' source] %n"
                + "Expecting:%n"
                + "  <\"text\">%n"
                + "to be equal to: %n"
                + "  <\"-\">"));
        ...
        try {
            assertThat(table).column("var12").value().isEqualTo("-");
            fail("An exception must be raised");
        } catch (AssertionError e) {
            Assertions.assertThat(e.getMessage()).isEqualTo(
                String.format("[Value at index 0 of Column at index 11 (column name : VAR12) of test table] %n"
                              + "Expecting:%n"
                              + "  <\"text\">%n"
                              + "to be equal to: %n"
                              + "  <\"-\">"));
