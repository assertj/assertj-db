...
public class AssertOnColumnType_IsDateTime_Test extends AbstractTest {
    ...
    public void test_is_date_time() {
        ...
        ChangeColumnAssert changeColumnAssert2 = assertThat(changes).change(1).column("var10");
        ChangeColumnAssert changeColumnAssertReturn2 = changeColumnAssert2.isDateTime(false);
        Assertions.assertThat(changeColumnAssert2).isSameAs(changeColumnAssertReturn2);
        TableColumnAssert tableColumnAssert1 = assertThat(table).column("var10");
        TableColumnAssert tableColumnAssertReturn1 = tableColumnAssert1.isDateTime(false);
        ...
        TableColumnAssert tableColumnAssertReturn2 = tableColumnAssert2.isDateTime(true);
    ...
    public void should_fail_because_value_have_different_type() {
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                "[Column at index 9 (column name : VAR10) of Change at index 0 (on table : TEST and with primary key : [5]) of Changes on tables of 'sa/jdbc:h2:mem:test' source] %n"
        ...
        try {
            ...
            Assertions.assertThat(e.getMessage()).isEqualTo(String.format(
                ...
                + "  <1>%n"
                + "to be of type%n"
                + "  <[DATE_TIME, NOT_IDENTIFIED]>%n"
                + "but was of type%n"
                + "  <NUMBER>"));
        }
        try {
            assertThat(table).column("var1").isDateTime(true);
            fail("An exception must be raised");
        } catch (AssertionError e) {
        ...
        try {
            assertThat(table2).column("var10").isDateTime(false);
            fail("An exception must be raised");
        } catch (AssertionError e) {
            Assertions.assertThat(e.getMessage())
