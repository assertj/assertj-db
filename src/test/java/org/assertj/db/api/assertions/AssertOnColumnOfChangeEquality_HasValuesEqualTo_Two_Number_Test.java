 ...
 * {@link org.assertj.db.api.assertions.AssertOnColumnOfChangeEquality#hasValuesEqualTo(Number, Number)} method.
 *
 * @author Régis Pouiller
 *
 */
public class AssertOnColumnOfChangeEquality_HasValuesEqualTo_Two_Number_Test extends AbstractTest {
  ...
  public void test_have_values_equal_to() {
    ...
    ChangeColumnAssert changeColumnAssert2 = changeColumnAssert.hasValuesEqualTo(2, 2);
  ...
  public void should_fail_because_value_at_start_point_is_different() {
    ...
    try {
      assertThat(changes).change().column("var3").hasValuesEqualTo(3, 3);
  ...
  public void should_fail_because_value_at_end_point_is_different() {
    ...
    try {
      assertThat(changes).change().column("var3").hasValuesEqualTo(2, 2);
      fail("An exception must be raised");
    } catch (AssertionError e) {
      Assertions.assertThat(e.getMessage()).isEqualTo("[Column at index 2 (column name : VAR3) of Change at index 0 (on table : TEST and with primary key : [1]) of Changes on tables of 'sa/jdbc:h2:mem:test' source] \n"
