...
public class AssertionsOnValueEquality_IsEqualTo_DateValue_Test extends AbstractTest {
  ...
  public void should_fail_because_value_is_not_equal_to() throws Exception {
    ...
    TableAssert tableAssert = assertThat(table);
    try {
      AssertionsOnValueEquality.isEqualTo(tableAssert, info, getValue(null, Date.valueOf("2007-12-24")), DateValue.of(2007, 12, 23));
  ...
  public void should_fail_because_value_is_not_a_date() throws Exception {
    ...
    TableAssert tableAssert = assertThat(table);
    try {
      AssertionsOnValueEquality.isEqualTo(tableAssert, info, getValue(null, 8), DateValue.of(2007, 12, 23));
      fail("An exception must be raised");
    } catch (AssertionError e) {
      Assertions.assertThat(e.getMessage()).isEqualTo(String.format("[description] %n"
                                                      + "Expecting:%n"
